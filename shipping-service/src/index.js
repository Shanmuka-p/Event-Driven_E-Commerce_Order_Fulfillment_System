...
async function setupSubscriptions() {
  await rabbitmq.subscribe(QUEUE_NAME, ['inventory.reserved'], { noAck: false }, async (msg, channel) => {
    ...
    try {
      ...
      // 4. Save ShipmentCreated to Outbox in same transaction
      ...
      console.log(`Saved ShipmentCreated event for Order: ${event.aggregateId} to outbox.`);

      await client.query('COMMIT');
      channel.ack(msg);
    } catch (err) {
      console.error('Error processing InventoryReserved event, rolling back:', err.message);
      await client.query('ROLLBACK');
